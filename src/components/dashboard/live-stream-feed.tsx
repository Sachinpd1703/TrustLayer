...
import { ChevronRight, ShieldCheck, Clock, Ban, Bot } from "lucide-react";
...
export function LiveStreamFeed({ events }: LiveStreamFeedProps) {
  ...
  // Initial mock event if no live events yet
  const displayEvents: LiveStreamEvent[] =
    events.length > 0
      ? events
      : [
          {
            ...
            decision: "ALLOW",
            reason: "Auto-approved within ₹5,000 spend cap & verified vendor whitelist.",
    ...
    <div className="rounded-xl border border-border bg-card shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between p-4 border-b border-border bg-secondary/20">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-mono text-muted-foreground">Streaming</span>
        </div>
      </div>
